...
3. **Use `TWeakObjectPtr` for all actor and component references** — message structs are value types that may outlive the broadcast frame if stored by a listener. `TObjectPtr` is unsafe in this context.
...
struct GAMECORE_API FProgressionLevelUpMessage
{
    ...
    UPROPERTY() TWeakObjectPtr<APlayerState> Instigator;
