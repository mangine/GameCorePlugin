...
**File:** `TimeWeatherTypes.h`  
**Part of:** Time Weather System

All core structs used by the Time Weather System. This header is included by the subsystem, data assets, and weather sequences. No Unreal module dependencies beyond `CoreUObject` and `GameplayTags`.

---

## FGameTimeSnapshot

Replicated once to clients on connect and on day rollover. Stateless — any holder can derive current time without callbacks.
...
struct GAMECORE_API FGameTimeSnapshot
{
    ...
    // NowSeconds should come from FPlatformTime::Seconds() (double).
...
**Note on time source:** All helpers take `double NowSeconds` from `FPlatformTime::Seconds()`. Do not cast to `int64` before passing — precision loss causes visible day-boundary glitches at high day counts.
...
Internal implementation detail of `UTimeWeatherSubsystem`. Computed at context registration from `UWeatherContextAsset::ComputeSeasonRanges`. Cached in `FContextState`; never serialised. Not exposed in public headers.
...
Built once per day by `ResolveSeasonContext` and passed into `UWeatherSequence::GetNextWeather`. Provides all situational data for deterministic weather selection.
...
struct FSeasonContext
{
    ...
    float        NormalizedDayTime  = 0.f;  // [0,1) at schedule build time (dawn)
    ...
    FGameplayTag LastWeather;               // tag of last resolved base weather (cross-day continuity)
};
```
...
The **sole output** of the Time Weather System. Downstream consumers read only this struct.
...
struct GAMECORE_API FWeatherBlendState
{
    ...
    // When a transition completes: A = B, BlendAlpha resets to 0, new B is set.
    // On first valid state: A = B = tag, BlendAlpha = 1.
    ...
    // Overlay event layer. OverlayAlpha drives how strongly the overlay
    // replaces/augments the base. Invalid tag = no overlay (OverlayAlpha = 0).
...
**Note:** The final keyframe covers through to `1.0` (end of day). The array is guaranteed sorted because `RebuildDailySchedule` builds it in ascending slot order.

---

## FActiveWeatherEvent

One in-flight overlay event. Lives inside `FContextState::ActiveEvents` (ticking) or `FContextState::QueuedEvents` (waiting).
...
**Cancel behaviour:** `CancelOverlayEvent` does **not** remove the event immediately. It clamps `SustainEnd = Now` and sets `FadeOutEnd = Now + ProportionalFade`, causing the event to begin fading on the next `TickAlpha`. Removal happens when `IsExpired` returns `true`.

---

## FRegionWeatherBlend

Convenience wrapper for the region-boundary blend helper. The area system computes `SpatialAlpha` and calls `UTimeWeatherSubsystem::GetBlendedWeatherState`. This struct is a future integration point.
...
struct GAMECORE_API FRegionWeatherBlend
{
    ...
    float              SpatialAlpha = 0.f; // 0 = pure A, 1 = pure B — driven by area system
};
```
...
Internal struct. Produced by `RollTimedEvents` once per day. Checked each tick by `TickScheduledTriggers`.
...
Array is sorted ascending by `TriggerNormTime` after construction, allowing the tick loop to break early.
