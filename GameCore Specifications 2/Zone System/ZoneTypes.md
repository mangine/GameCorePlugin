...
**File:** `GameCore/Source/GameCore/Zone/ZoneTypes.h`

Pure data types. No gameplay logic. Included by all Zone System files.
...
Serializable shape descriptor used for runtime zone spawning. Not used for editor-placed zones (those are configured directly on the shape components).
...
> **Replication note:** Mark the containing `AZoneActor` property with `ReplicatedUsing=OnRep_DynamicState`. UE's standard struct delta serialisation handles bandwidth-efficient replication automatically.
