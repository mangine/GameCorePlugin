...
**Files:** `Inventory/InventoryTypes.h`, `Inventory/IItemDefinitionProvider.h`
...
struct GAMECORE_API FInventorySlot : public FFastArraySerializerItem
{
    ...
    // Matrix extension: FIntPoint Position / FIntPoint Size will be appended here;
    // SlotIndex remains the replication key and the persistence key.
    UPROPERTY()
...
> **`PostReplicatedChange` note:** Resolve the outer component via `GetTypedOuter<UInventoryComponent>()` inside the implementation. Do not store a raw pointer to the component inside the struct.
...
> **Null safety:** `UInventoryComponent` always null-checks the provider. A missing provider is not a shipping error — it silently degrades (zero weight, stack size 1). In development builds, `ensure(Provider)` fires once at `BeginPlay` if a `UWeightConstraint` is active and no provider is set.
