...
+GameplayTagList=(Tag="GameCoreEvent.Inventory.ItemRemoved",    DevComment="Item removed from an inventory slot")
...
Also fires `RequirementEvent.Inventory.ItemAdded` via the watcher subsystem so `URequirement_HasItem` implementations are re-evaluated.
...
**Scope:** `ServerOnly`
**Origin:** `UInventoryComponent::TryPlaceAt` / `TryPlaceAuto` when a blocking result is returned.

Useful for UI feedback and analytics, not for blocking logic (the result enum already covers that).
