...
`URequirement_Composite` is the boolean logic layer of the Requirement System. It holds a tree of child `URequirement` instances and combines their results using AND, OR, or NOT. It is itself a `URequirement` subclass — composites nest arbitrarily.
...
enum class ERequirementOperator : uint8
{
    ...
    // Pass → Fail (using NotFailureReason). Fail → Pass.
...
public:
    ...
    // Failure reason used when NOT fails (child passed).
    ...
    const FRequirementContext& Context) const
{
    ...
    // Allows event-only children to participate correctly.
    switch (Operator)
    {
    case ERequirementOperator::AND:
    {
...
FailureReason propagates up unchanged through nested composites. The innermost failing leaf's reason surfaces to the consuming system. Composites never wrap or replace child failure reasons except for NOT.
...
`URequirementLibrary::ValidateRequirements` checks composites recursively at `BeginPlay` in dev builds:
- NOT composites must have exactly one child.
- No null children at any nesting depth.
- Authority mismatches if `bRequireSync` is passed.

Cycle detection is not performed at runtime. Circular composites constructed programmatically will recurse infinitely. The Details panel cannot produce cycles since instanced UObjects cannot self-reference.
