...
**Files:** `TimeWeatherConfig.h`  
**Part of:** Time Weather System

Two classes: `UTimeWeatherProjectSettings` (project settings bridge) and `UTimeWeatherConfig` (data asset holding all global parameters).

---

## UTimeWeatherProjectSettings

A `UDeveloperSettings` subclass. Appears in **Project Settings → GameCore → Time Weather**. The subsystem calls `GetDefault<UTimeWeatherProjectSettings>()` in `Initialize` to locate the config asset.
...
`UDataAsset` holding all global time and weather parameters. Referenced by `UTimeWeatherProjectSettings`. Contains no per-frame logic.
...
public:
    ...
    // Common: 1200 (20 min), 2400 (40 min), 4800 (80 min).
    ...
    // The global weather context. Must not be null — the subsystem will
    // log an error and disable itself if unset.
    UPROPERTY(EditDefaultsOnly, Category="Weather")
